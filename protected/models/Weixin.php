		...
		if (!empty($postStr)){
				...
				if($msgType=='text'){
            		...
            		for($i=0;$i<count($rs);$i++){
            			...
            			$meters = "(距离约" . $meter ."米)";
            			$datas[$meter] = array('title'=>$rs[$i]['name'].$meters,'description'=>$rs[$i]['name'],'picUrl'=>Yii::app()->request->hostInfo.'/'.Yii::app()->request->baseUrl.'/'.$rs[$i]['picUrl'],'url'=>'http://flvwechat.samesamechina.com/lv_wechat/store.html'); 
            		}
					ksort($datas);
					$i=0;
